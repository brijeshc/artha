...
- [ ] MCP serves inferred facts labeled, below vouched facts in budget order; never as certified; rationale is never synthesized. *(next 21b slice - the layer is loaded into the MCP query index but `rank.ts` does not yet surface it.)*
...
- [ ] **21b-2 - transitions + flow-step text.** Extend synthesis + the verifier to state-machine transitions (both states must be real enum members; the trigger stays grounded) and flow-step descriptions, with the storage + render they need.
  - [x] **flow-step text (2026-07-17).** A flow's reached modules (already extracted by 21a as `inferred_steps`) each get one synthesized line - what the flow does there, read from the entry's pinned code - keyed back to the real module so a hallucinated step is dropped, verifier-gated on the same tier as the name/summary, stored in a new `note` column that the build overlays and reverts on drift (D12), and rendered as a moonlight list beside each module. The *order* is never synthesized (D8). Cache bumped to v2.
  - [ ] **state-machine transitions.** The remaining half: 21a extracts no transitions today (states only - transitions were left blank as human delta), so this needs a decision on whether the model proposes the transition structure (from/to/trigger, verifier-checked: both states real, trigger grounded) or a deterministic 21a transition extractor lands first, plus its storage, verifier rules, render (incl. the 23e-1 chalk lifecycle), and materialize interaction.
- [ ] **21b-3 - MCP serving.** `rank.ts` surfaces inferred facts labeled (`[machine-described, unverified by team]`), below vouched facts in budget order; never certified; rationale never synthesized.
