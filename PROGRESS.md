...
| 21b  | Inferred layer - LLM synthesis      | 🟡 21b-1 done | opt-in, spend-capped enrichment + verification gate. **21b-1 (2026-07-16)**: `artha infer` - the pluggable synthesis pipeline (`api`/`claude-cli`, mirroring T06), names+summaries over the 21a candidates, incremental content-hash cache (`.artha/.inferred.json`), a deterministic verifier that downgrades ungrounded claims to `uncertain`, and a build overlay that reverts on drift (D12); OQ-C locked (single strong model, keep+mark uncertain). Remaining: 21b-2 transitions+step text · 21b-3 MCP serving · 21b-4 the `uncertain` render |
