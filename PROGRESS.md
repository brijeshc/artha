...
| 21b  | Inferred layer - LLM synthesis      | 🟡 21b-1/2/3 done | opt-in, spend-capped enrichment + verification gate. **21b-1 (2026-07-16)**: `artha infer` - the pluggable synthesis pipeline (`api`/`claude-cli`, mirroring T06), names+summaries over the 21a candidates, incremental content-hash cache (`.artha/.inferred.json`), a deterministic verifier that downgrades ungrounded claims to `uncertain`, and a build overlay that reverts on drift (D12); OQ-C locked (single strong model, keep+mark uncertain). **21b-2 (2026-07-17)**: flow-step text (a flow's reached modules each get one grounded, verifier-gated line, never the order) + **state-machine transitions** (a cross-file `memberUsages` state-usage index feeds the synthesizer; the model proposes {from,to,trigger}, a gate drops fabricated-state edges and grounds each trigger; `artha_inferred_transitions` overlaid + reverting on drift; vouching seeds the human transitions draft; rendered as a moonlight list). Cache v2→v3. **21b-3 MCP serving (2026-07-17)**: `context_for_task` serves the inferred layer labeled `[machine-described, unverified by team]`, ranked in its own FTS corpus (`artha_inferred_fts`, so human bm25 is untouched) and seated strictly below vouched facts in the budget; on by default (`include_inferred`), never certified, rationale never synthesized. Remaining: 21b-4 the `uncertain` render |
...
- **T21b-2 (state-machine transitions)** done - the other half of 21b-2 ([tasks-v0.3/21-inferred-layer.md](tasks-v0.3/21-inferred-layer.md) §21b), and **task 21b-2 is now complete**. A machine-read concept could show its *states* (21a) but never how it *moves* between them - transitions were left blank as the human delta. Now, where the code shows a move, it is read, verified, and drawn. Decided with the developer: **the model proposes, a deterministic gate grounds** (not a deterministic edge extractor - directed from→to edges with triggers need dataflow the project cut), fed by a new **cross-file state-usage index**.
  - **The usage index (deterministic, 21a).** A new resolver seam, `memberUsages(file, state)` (`SymbolResolver` + tree-sitter): the declarations in a file that *use* a state - a union member as a string literal in an assignment/comparison/`case`/initializer/return, or an `Enum.Member` access - each rolled up to its enclosing resolvable declaration, precision over recall (a member word in bare prose does not count). `inferLayer` scans **every** source file for each concept and pins the sites as `usage` evidence (capped, deterministic), so the code that moves a state - usually in a *different* file from the type - is now pinned, drift-hashed, and revealable (D5) through the same machinery as any pin. (A web-tree-sitter gotcha cost a first draft: node wrappers are fresh objects per access, so `childForFieldName('right') === lit` never holds - the context check now reads the parent's *type*, since a string literal under an assignment/field/declarator is always the value.)
  - **Synthesis + the gate (21b).** `synthesize` takes the concept's `members` and returns `transitions` ({from, to, trigger}); the prompt asks for only the moves the code shows, using the exact state names, never completing the diagram. Two gates: `infer.ts` **drops** any edge whose from/to is not a real state (a fabricated state is false, not uncertain), and `verify.ts` grounds each surviving **trigger** against the pinned evidence on the concept's one confidence tier (an ungrounded trigger → the whole concept `uncertain`). Stored in a new `artha_inferred_transitions` table; cache **v3**; `build` overlays them (creating them - 21a emits none) only where the evidence still matches, reverting on drift (D12) like everything else.
  - **Materialize (the payoff).** Vouching an inferred concept now seeds its grounded transitions into the human `Concept.transitions` draft (`materializeInferred`) - reading-is-reviewing hands over a real diagram to correct, not a blank one.
  - **Render - a moonlight list, decided by the live E2E.** The first cut drew the transitions in the 23e-1 chalk lifecycle (in moonlight); the E2E on real machine output (7 transitions, `cancel()` reaching `cancelled` from every state, verbose triggers) showed the diagram tangling into an unreadable band - the chalk layout is built for a few short *human* triggers. So an inferred concept lists its transitions in moonlight (`Transitions read from code`: the edge in code voice, the trigger in machine prose beside it) - robust to many long triggers, readable in five seconds. A happy consequence: the chalk diagram stays human-only, so **23e-1's "every concept arrow is human ink" holds unchanged**.
  - **Verified**: typecheck (CLI + web) + Biome clean; **508 tests pass** (+23: resolver usage scan incl. cross-file / declaration-excluded / non-value-context-excluded / enum-access / every-ref-resolves; 21a usage pins; prompt lists States; parse transitions; verifier grounds a trigger; align drops a fabricated-state edge; cache v3 round-trip + v2 discard; overlay + drift revert; materialize seeds transitions; the moonlight list renders). **Live E2E** with the real `claude-cli` engine on the demo (its `Checkout` now moves the `OrderState` field across four methods): `infer` synthesized **"Order lifecycle"** with **7 grounded transitions** read from `Checkout.ts` (a different file from the `OrderState` declaration) - `cart→placed` "place() is called while in cart", `placed→paid` "…charges the gateway and sends an email", `paid→fulfilled`, and `cancel()` from every state, all verified `inferred`; `build` overlaid them into the index; the served page listed all seven in moonlight, cleanly.
  - **Remaining on T21b**: 21b-4 (the `uncertain` dimmest-moonlight render). Then 22 (contradiction view) → T18.

- **T21b-3 - MCP serves the inferred layer** done - the slice that finally hands the machine layer to an agent ([tasks-v0.3/21-inferred-layer.md](tasks-v0.3/21-inferred-layer.md) §21b). Until now the inferred layer lit the dashboard but `context_for_task` still answered from vouched facts alone, so the -56%-discovery value never reached a repo the team had not curated. Now it does - **labeled, and always below vouched**.
  - **A separate FTS corpus** (`artha_inferred_fts`, `src/build/db.ts`): the machine layer is ranked lexically in its **own** FTS5 table, never folded into `artha_fts`. Folding it in would have shifted bm25's IDF and silently re-ranked human facts - so a stranger's inferred prose can never perturb the team's own retrieval. `query.ts` exposes `inferredFts()` beside `fts()` (the runner is now table-parameterized), defensive on a pre-21b-3 index (no table → empty, never a throw).
  - **`rankInferred`** (`src/mcp/rank.ts`): the same lexical + structural blend as `rankFacts`, minus the semantic term (inferred facts carry no T14 vectors - the term is simply absent, as structural is when no symbols/files are given), weighted by confidence so a downgraded `uncertain` fact sinks below a grounded one. `formatInferredItem` tags every one `[machine-described, unverified by team]` (`uncertain` → `[machine-described, uncertain]`) - **never** `[certified]`.
  - **Vouched first, machine-described strictly below** (`contextBundle`): human facts fill the budget (`selectWithinBudget`); the inferred tier gets only the **remainder** (`selectInferredWithinBudget` via a shared greedy packer, seeded with the tokens already spent), so the machine layer can never crowd out the team's own context. On a repo with nothing vouched, the top inferred item is still forced, so an un-curated repo says something useful (D1: value before ask). `include_inferred` (default **true**) opts the whole tier out; `rationale is never synthesized` holds by construction - `why` stays human-only, and an inferred fact has no `why` to serve.
  - **The tool now says so** (`src/mcp/server.ts`): `context_for_task` retitled "Product context for a task", its description and the server's `initialize` instructions naming the labeled machine layer below the vouched facts; a new `include_inferred` input.
  - **Verified**: typecheck (CLI + web) + Biome clean; **495 tests pass** (+10: `rankInferred` on its own corpus / structural fallback / uncertain-sinks / drops-zero-relevance / empty-layer; `formatInferredItem` labels + never-certified + pins + uncertain; `contextBundle` includes-labeled-below-vouched / `include_inferred:false` omits / serves-when-nothing-vouched; the MCP wire serves it labeled and honors the flag; the cold message reworded). **Live E2E** on a real tree-sitter build (checkout flow + `OrderState` union + a certified Stripe decision): `context_for_task "charge the card at checkout"` returned `[certified] decision.stripe` first, then `[machine-described, unverified by team]` for the `Order State` concept and the `Place Order` flow below it; `include_inferred:false` returned the certified fact alone; and `fts('card')` returned only the human fact - the inferred corpus left human bm25 untouched.
  - **Remaining on T21b**: 21b-2's other half (state-machine transitions), 21b-4 (the `uncertain` dimmest-moonlight render on the dashboard). Then 22 (contradiction view) → T18.

- **T21b-2 (flow-step text)** done - the first half of 21b-2 ([tasks-v0.3/21-inferred-layer.md](tasks-v0.3/21-inferred-layer.md) §21b), the piece that lets a machine-read flow say *what* it does at each module it reaches - without ever claiming the *order*, which stays the human's delta (D8). (State-machine transitions, 21b-2's other half, remain.)
  - **The candidate already carried the steps.** 21a rolls a flow's import fan-out up to module altitude (`inferred_steps`); 21b-1 described the flow as a whole but left each step a bare label. Synthesis now takes the reached modules as input (`SynthStep`) and returns one grounded line per module (`SynthStepText`) - "charges the customer's card", "sends an order confirmation" - read from the entry's own pinned code. The prompt asks for step text *only* when the candidate lists "Reaches" modules (a flow), and forbids describing the order (`SYNTH_SYSTEM_PROMPT`, `renderSynthPrompt`).
  - **The verifier grades the step text too** (`src/infer/verify.ts`): the name, the summary, and every step line share one confidence tier, so a single ungrounded code assertion anywhere - `writes to \`KafkaTopic\`` on a flow that touches no such thing - downgrades the whole fact to `uncertain`. Plain grounded prose passes.
  - **Alignment drops what the flow doesn't reach** (`src/infer/infer.ts`): the model's steps are matched back to the real 21a fan-out by module id, so a hallucinated or mis-keyed step (`src/nowhere`) is dropped *before* verification and storage - a stray step never taints the tier and never reaches the reader.
  - **Storage + overlay, drift-honest** (`src/build/db.ts`, `src/build/build.ts`, `src/analytics/inferred.ts`): a new nullable `note` column on `artha_inferred_steps`, filled by the build overlay from the cache - but only for a flow whose evidence hash still matched, so a drifted flow's step notes revert to bare labels alongside its reverted prose (D12). The cache bumps to **v2** (`src/infer/cache.ts`); a v1 cache is discarded and re-inferred rather than misread.
  - **The render reads as a list, not a chip row** (`web/src/components/Inferred.tsx`, `styles.css`, `api.ts`): once any step carries a note the fan-out lays out one module per line with its synthesized description beside it in moonlight (`moon-steps-noted`); the order stays unstated. The `InferredStepView.note` slot threads server → API → web with zero other client change.
  - **Verified**: typecheck (CLI + web) + Biome clean; **485 tests pass** (+6: the prompt asks for per-module descriptions when a flow reaches modules; the parser reads step text and drops malformed entries; the verifier downgrades an ungrounded step; the cache round-trips step text, bumps to v2 and discards v1; the orchestrator synthesizes step text and drops steps for modules never reached; the build overlays step notes into the index and reverts them on drift; the page renders a step's description beside its module). A live `artha infer` E2E on the demo (a paid LLM call) is the recommended check before merge, matching the 21b-1 norm.
  - **Remaining on T21b**: 21b-2's other half (state-machine transitions, verifier-gated), 21b-3 (MCP serves the layer labeled below vouched), 21b-4 (the `uncertain` dimmest-moonlight render). Then 22 (contradiction view) → T18.

### 2026-07-16

- **T21b-1 - the LLM synthesis pipeline** done - the first slice of 21b ([tasks-v0.3/21-inferred-layer.md](tasks-v0.3/21-inferred-layer.md) §21b), and the piece that turns the deterministic 21a layer *readable*. `artha infer` enriches a machine-read candidate's mechanical name + prose into product language a non-author can read aloud - opt-in, spend-capped, verified against the code, and re-derivable for free.
  - **OQ-C locked with the developer** (the spec reserved it for 21b's start): a **single strong model** (`infer.model`, default `claude-opus-4-8` - no cheap card tier for now, add one only if steady-state cost proves it); the synthesis **scope reaches transitions + flow-step text** (a later slice - 21b-1 does names + summaries, the whole pipeline); and an unverifiable claim is **kept and marked `uncertain`**, never silently dropped (the label carries the honesty).
  - **The engine, mirroring T06** (`src/infer/*`): an `Inferrer` interface with two shipped engines (`api` via the Anthropic SDK with structured output; `claude-cli` reusing the Claude Code login) and a shared prompt / schema / parser - the exact shape of the miner, down to the injected stub in tests. The miner's real CLI runner + credential/binary checks are reused, not re-implemented (`realCliRunner`, `requireApiAuth`, `assertClaudeCli`).
  - **`artha infer`** (`src/infer/infer.ts` + `src/commands/infer.ts`): re-derives the *same* 21a candidates `build` emits (shared `collectPins`, same files/resolver/refs/human-pin suppression), keeps every still-valid cached synthesis, and sends only the rest to the engine up to a per-candidate cap (`--max`, default 50). Incremental by a **content-hash cache** (`.artha/.inferred.json`) keyed by the fact id + the hash of its pins' code - an unchanged candidate costs zero spend, a steady state costs nothing. `--dry-run` previews with no engine and no credentials.
  - **The verifier gate** (`src/infer/verify.ts`, deterministic, offline): the spec's "checker validates claim-vs-pinned-code" made real without a second model. It grounds every *code-shaped* assertion in the synthesis - backtick-quoted spans and identifier-shaped words (camelCase / snake_case / letter-digit) - against the pinned evidence by word-pieces. Plain product language passes (D4's "Subscription lifecycle" over `SubscriptionStatus`); a foreign term (`DynamoDB`, `chargeStripe`) whose pieces are nowhere in the code does not, and is downgraded to `uncertain`. No resolvable evidence at all → `uncertain`. Two tiers out (D7): `inferred` (grounded) or `uncertain` (kept, labeled).
  - **The build overlay** (`src/build/build.ts`): after the 21a layer is derived, `build` overlays the cache onto each fact's `heading` (name), `body` (prose), and `confidence` - **only where the pins' evidence hash still matches**. Drifted code silently falls back to the deterministic 21a text (D12 - moonlight regenerates quietly, nothing to maintain). A failed/absent `infer` engine leaves 21a untouched because the overlay applies only what was cached. The enriched prose flows into the exact `describedAs`/body slot 23a reserved, so the dashboard shows it with zero client rework (the `inferred`/`uncertain` tiers already word themselves via `INFERRED.confidence`).
  - **Verified**: typecheck (CLI + web) + Biome clean; **479 tests pass** (+33: verifier grounds/downgrades/empty-evidence + word-piece recombination; parse tolerates fences + refuses partial; cache round-trip / mangled-never-throws / version-guard / drop-malformed / sorted-byte-stable + evidenceHash stable/drift/order-independent/unresolvable; orchestrator synthesizes-all / downgrades-a-planted-false-claim / incremental-reuse-zero-spend / spend-cap / dry-run-no-creds / declines; build overlays + reverts-on-drift + intact-without-infer; config parses `infer:`). **Live E2E** via the real `claude-cli` engine on the demo (`infer --max 1`): the `OrderState` union synthesized to **"Order lifecycle"** with a grounded summary that itself notes the transition rules "are not defined here" (respecting D8), verified `inferred`; `artha build` overlaid it into `artha_inferred` while the other 8 candidates stayed `read-from-code`.
  - **Remaining on T21b**: 21b-2 (transitions + flow-step text, verifier-gated), 21b-3 (MCP serves the layer labeled below vouched), 21b-4 (the `uncertain` dimmest-moonlight render). Then 22 (contradiction view) → T18.

- **T23e-4 - more trace entry points** done - the last slice of the atlas elevation ([tasks-v0.3/23-atlas-elevation.md](tasks-v0.3/23-atlas-elevation.md) §23e). **Task 23 is complete.**
  - **The trace where a flow is named.** A flow is a path across the board, so offering the trace only on the flow's own page buried it. A navigator flow row and a catalog flow card now each carry a quiet `⤳` - dim until you go looking, always reachable by keyboard. The card is one big link, so the trace rides a slot *alongside* it (an anchor inside an anchor is not markup any browser agrees on) - a `cap-card-slot` wrapper, the trace absolutely seated top-right. A concept is not a path across anything, so it offers nothing.
  - **A machine-read flow traces as what it *reaches*, never as a route** (decided with the developer). 21a reads a flow's entry point off the code and rolls its import fan-out up to module altitude, then deliberately stops: the order those steps run in is the human's delta. The board's route is a **numbered sequence**, so tracing a skeleton with it would dress a source-order artifact up as a runtime one - precisely the claim 21a refuses to invent. So `inferredFlowTrace` marks the **entry** (`⌂`, the one station the code really does state - it is the flow's single evidence pin) and lights the rest as *touched*: dashed **moonlight** legs fanning out **from the entry** rather than chaining one to the next, **no numbers anywhere**, and a card headed "what this reaches" that says it out loud - *"Read from code, so this is what it touches - not the order it runs in."* `FlowTrace` gained `kind: 'route' | 'reaches'`; a vouched flow keeps its numbered stations and status colour untouched.
  - **Verified**: typecheck (CLI + web) + Biome clean; **446 tests pass** (+6: the reach derivation is entry-first, lists each module once, numbers nothing, and stands as `described`; the board draws dashed moonlight legs + the entry badge and no step numbers; the card refuses an order and opens the moonlight page, not a capability page; the nav row and the catalog card offer the trace; a concept offers none; the card's trace is the slot's sibling, never the link's child). Web bundle 81.5 KB gzip JS. **Live E2E** on the demo: `#/?f=inferred:flow:…placeOrder` lights `checkout` with `⌂` and fans dashed moonlight to `billing` and `notifications`, both unnumbered, the card reading "3 modules reached" above the unordered list.
  - **T23 (atlas elevation) is done.** Next: 21b (LLM synthesis) → 22 (contradiction view) → T18.
