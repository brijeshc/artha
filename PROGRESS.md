...
| 21b  | Inferred layer - LLM synthesis      | 🟡 21b-1 + step text + MCP | opt-in, spend-capped enrichment + verification gate. **21b-1 (2026-07-16)**: `artha infer` - the pluggable synthesis pipeline (`api`/`claude-cli`, mirroring T06), names+summaries over the 21a candidates, incremental content-hash cache (`.artha/.inferred.json`), a deterministic verifier that downgrades ungrounded claims to `uncertain`, and a build overlay that reverts on drift (D12); OQ-C locked (single strong model, keep+mark uncertain). **21b-2 flow-step text (2026-07-17)**: a flow's reached modules each get one grounded, verifier-gated line (what it does there, never the order); `note` column overlaid + reverting on drift; cache v2. **21b-3 MCP serving (2026-07-17)**: `context_for_task` serves the inferred layer labeled `[machine-described, unverified by team]`, ranked in its own FTS corpus (`artha_inferred_fts`, so human bm25 is untouched) and seated strictly below vouched facts in the budget; on by default (`include_inferred`), never certified, rationale never synthesized. Remaining: 21b-2 state-machine transitions · 21b-4 the `uncertain` render |
