...
| 21c  | Inferred layer - dashboard reframe  | 🟡 partial   | two-light grammar, prose-first pages, delta band, worded confidence shipped; flow (Reaches) + convention (Symbols that match) moonlight pages + module/catalog sections shipped in slice 2; **KPI reframe (D11) shipped via 23a**; **evidence revealed (D5) shipped via 23d-1**; **vouch-by-reading + materialize-on-touch (D9 core, OQ-A) shipped via 23d-2**; **the review walk (D9) shipped via 23d-3**; the delta band on human pages (D6) + value-ranked queue (D10) remain - delivered by the rest of 23d |
| 22   | Contradiction view                  | ⬜            | inferred vs certified |
| 23   | Atlas elevation                     | 🟡 23a+23b+23c+23d-1+23d-2+23d-3 done | honest KPIs (% vouched / % described), the **blackboard Board** (handmade flowchart, chalk = light), **inner boards** (23b - drill a module into its own blackboard), **the observatory** (23c - three hand-rolled instrument charts on a fourth nav view, dataviz method), **evidence revealed** (23d-1 - every pin reveals its exact source lines, D5), **vouch-by-reading** (23d-2 - an inferred concept/flow vouches/edits in place, materializing into a real entry with `derived_from` provenance, D9 core + OQ-A), and **the review walk** (23d-3 - press R on any module/capability page to sweep its unvouched claims one at a time, claim left + code right, one keystroke to vouch or correct, D9); next: the delta band (D6) + value queue (D10) to finish 23d, craft debt (23e) - see [tasks-v0.3/23-atlas-elevation.md](tasks-v0.3/23-atlas-elevation.md) |
